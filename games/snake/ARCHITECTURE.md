...
When adding a skin, provide its standard image resources, register it in `content/skins.js`, and add a `renderProfile` only when its art needs one. No `Snake.js`, collision or `Game.js` changes are required for a large head or wide body.
