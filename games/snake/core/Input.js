...
export class Input {
  constructor({ target, wrap, directions, swipeMin, onAction, onEscape, isPaused }) {
    this.target=target;this.wrap=wrap;this.directions=directions;this.swipeMin=swipeMin;this.onAction=onAction;this.onEscape=onEscape;this.isPaused=isPaused;this.pointerStart=null;
  ...
  keydown(event){if(isEditableTarget(event.target))return;if(event.key==='Escape'&&this.onEscape()) {event.preventDefault();return;}if(event.code==='Space'){event.preventDefault();this.onAction('PAUSE');return;}if(event.key==='r'||event.key==='R'){event.preventDefault();this.onAction('RESTART');return;}if(event.key==='b'||event.key==='B'){event.preventDefault();this.onAction('BUTTON_B');return;}if((event.key==='a'||event.key==='A')&&this.isPaused?.()){event.preventDefault();this.onAction('BUTTON_A');return;}const name=this.keyMap[event.key];if(name){event.preventDefault();this.onAction(`MOVE_${name.toUpperCase()}`);}}
